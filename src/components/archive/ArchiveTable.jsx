...
export function ArchiveTable({ rows, totalRows, currentPage, rowsPerPage, pageSizeOpen, onPageChange, onRowsPerPageChange, onTogglePageSize }) {
  ...
  return (
    <div className="panel archive-table">
      <div className="archive-table__scroll">
        <table className="archive-table__grid">
          <colgroup>
            <col className="archive-table__col archive-table__col--time" />
            <col className="archive-table__col archive-table__col--icon" />
          ...
          <thead>
            <tr>
              <th>Время</th>
              <th className="archive-table__head-spacer" aria-hidden="true" />
              <th>Объект</th>
              <th>Координаты</th>
              <th>Параметр</th>
              <th>Значение</th>
          ...
          <tbody>
            {pageRows.length === 0 ? (
              <tr>
                <td colSpan={7}>Нет записей за выбранный период</td>
              </tr>
            ) : pageRows.map((row, index) => (
              <tr key={`${row.time}-${row.object}-${index}`}>
                <td>{row.time}</td>
                <td><img className="archive-table__object-icon" src={row.icon} alt="" /></td>
                <td>
